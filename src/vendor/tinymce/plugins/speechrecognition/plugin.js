...
(function () {
	...
	var startSpeechRecognition = function (editor, startedState, currentLanguageState, SpeechRecognition) {
		...
		recognizer.addEventListener('end', () => {
			...
			if (isListening) {
			  try {
			  	// write the text natuarally
				const node = tinymce.activeEditor.selection.getNode();
				const win = tinymce.activeEditor.getWin();

				// make sure node is a valid block element, not body or page div
				if (!node || node.nodeName === 'BODY' || node.classList.contains('page')) {
					//console.log('cursor not inside a valid element');
					return;
				}
				...
				//console.log('lines:', lineCount);
			  	...
			  	if(lineCount > 5)
			  	{
					...
					const p = tinymce.activeEditor.dom.create('p');
					p.innerHTML = '&nbsp;';
					
					// insert after current node, not at end of parent
					tinymce.activeEditor.dom.insertAfter(p, node);
					
					// Move cursor into new paragraph
					tinymce.activeEditor.selection.setCursorLocation(p, 0);
