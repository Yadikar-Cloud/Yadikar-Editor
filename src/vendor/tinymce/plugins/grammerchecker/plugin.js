...
(function () {
    ...
    var hasProPlugin = function (editor) {
      if (editor.hasPlugin('tinymcegrammerchecker', true)) {
        if (typeof window.console !== 'undefined' && window.console.log) {
          window.console.log('Grammer Checker Pro is incompatible with Grammer Checker plugin! ' + 'Remove \'grammerchecker\' from the \'plugins\' option.');
        }
        return true;
      } else {
        return false;
    ...
    var fireGrammercheckStart = function (editor) {
      return editor.fire('GrammercheckStart');
    };
    var fireGrammercheckEnd = function (editor) {
      return editor.fire('GrammercheckEnd');
    };

    var getLanguages = function (editor) {
      var defaultLanguages = 'English=en,Danish=da,Dutch=nl,Finnish=fi,French=fr_FR,German=de,Italian=it,Polish=pl,Portuguese=pt_BR,Spanish=es,Swedish=sv';
      return editor.getParam('grammerchecker_languages', defaultLanguages);
    };
    var getLanguage = function (editor) {
      var defaultLanguage = editor.getParam('language', 'en');
      return editor.getParam('grammerchecker_language', defaultLanguage);
    };
    var getRpcUrl = function (editor) {
      return editor.getParam('grammerchecker_rpc_url');
    };
    var getGrammercheckerCallback = function (editor) {
      return editor.getParam('grammerchecker_callback');
    };
    var getGrammercheckerWordcharPattern = function (editor) {
      var defaultPattern = new RegExp('[^' + '\\s!"#$%&()*+,-./:;<=>?@[\\]^_{|}`' + '\xA7\xA9\xAB\xAE\xB1\xB6\xB7\xB8\xBB' + '\xBC\xBD\xBE\xBF\xD7\xF7\xA4\u201D\u201C\u201E\xA0\u2002\u2003\u2009' + ']+', 'g');
      return editor.getParam('grammerchecker_wordchar_pattern', defaultPattern);
    };
    ...
    var DomTextMatcher = function (node, editor) {
      ...
      var hasClass = function (elm) {
        return elm.className.indexOf('mce-spellchecker-grammar') !== -1;
      };
    ...
    var defaultGrammercheckCallback = function (editor, pluginUrl, currentLanguageState) {
      return function (method, text, doneCallback, errorCallback) {
		const url = new global$1(pluginUrl).toAbsolute(getRpcUrl(editor));
		...
		.then(parseResult => {
		  if (!parseResult) {
			// Handle case where response is not proper JSON
    ...
    var sendRpcCall = function (editor, pluginUrl, currentLanguageState, name, data, successCallback, errorCallback) {
      var userGrammercheckCallback = getGrammercheckerCallback(editor);
      var GrammerCheckCallback = userGrammercheckCallback ? userGrammercheckCallback : defaultGrammercheckCallback(editor, pluginUrl, currentLanguageState);
      GrammerCheckCallback.call(editor.plugins.grammerchecker, name, data, successCallback, errorCallback);
    };
    var grammercheck = function (editor, pluginUrl, startedState, textMatcherState, lastSuggestionsState, currentLanguageState) {
      ...
      sendRpcCall(editor, pluginUrl, currentLanguageState, 'grammercheck', getTextMatcher(editor, textMatcherState).text, successCallback, errorCallback);
      editor.focus();
    };
    var checkIfFinished = function (editor, startedState, textMatcherState) {
      if (!editor.dom.select('span.mce-spellchecker-grammar').length) {
        finish(editor, startedState, textMatcherState);
      }
    };
    ...
    var ignoreWord = function (editor, startedState, textMatcherState, word, spans, all) {
      ...
      if (all) {
        global$2.each(editor.dom.select('span.mce-spellchecker-grammar'), function (span) {
    ...
    var markErrors = function (editor, startedState, textMatcherState, lastSuggestionsState, data) {
      ...
      var bookmark = editor.selection.getBookmark();
	  const obj = {};
      getTextMatcher(editor, textMatcherState).find(getGrammercheckerWordcharPattern(editor)).filter(function (match) {
		return suggestions.some(function (suggestion) {
			if(suggestion.offset === match.start) {
				return true;
			} else {
				return false;
			}
		...
		suggestions.forEach(suggestion => {
			if(suggestion.offset === match.start) {
				obj[match.text] = suggestion;
			}
		});
        return editor.dom.create('span', {
          'class': 'mce-spellchecker-grammar',
    ...
    var register$1 = function (editor, pluginUrl, startedState, textMatcherState, lastSuggestionsState, currentLanguageState) {
      editor.addCommand('mceGrammerCheck', function () {
        grammercheck(editor, pluginUrl, startedState, textMatcherState, lastSuggestionsState, currentLanguageState);
      });
    };
    ...
    var grammercheckerEvents = 'GrammercheckStart GrammercheckEnd';
    ...
    var register = function (editor, pluginUrl, startedState, textMatcherState, currentLanguageState, lastSuggestionsState) {
      ...
      var startGrammerchecking = function () {
        grammercheck(editor, pluginUrl, startedState, textMatcherState, lastSuggestionsState, currentLanguageState);
      };
      var buttonArgs = {
        tooltip: 'Grammercheck',
        onAction: startGrammerchecking,
        icon: 'spell-check',
        onSetup: function (buttonApi) {
          var setButtonState = function () {
            buttonApi.setActive(startedState.get());
          };
          editor.on(grammercheckerEvents, setButtonState);
          return function () {
            editor.off(grammercheckerEvents, setButtonState);
          };
        }
      };
      ...
      if (languageMenuItems.length > 1) {
        editor.ui.registry.addSplitButton('grammerchecker', splitButtonArgs);
      } else {
        editor.ui.registry.addToggleButton('grammerchecker', buttonArgs);
      }
      editor.ui.registry.addToggleMenuItem('grammerchecker', {
        text: 'Grammercheck',
        icon: 'spell-check',
        onSetup: function (menuApi) {
          ...
          editor.on(grammercheckerEvents, setMenuItemCheck);
          return function () {
            editor.off(grammercheckerEvents, setMenuItemCheck);
          };
        },
        onAction: startGrammerchecking
      });
    ...
    var getSuggestions = function (editor, pluginUrl, lastSuggestionsState, startedState, textMatcherState, currentLanguageState, word, spans) {
	  ...
	  const replacements = suggestions.replacements.map(item => item.value);
      global$2.each(replacements, function (replacement) {
        items.push({
          text: replacement,
          onAction: function () {
    ...
    var setup = function (editor, pluginUrl, lastSuggestionsState, startedState, textMatcherState, currentLanguageState) {
      var update = function (element) {
        var target = element;
        if (target.className === 'mce-spellchecker-grammar') {
      ...
      editor.ui.registry.addContextMenu('grammerchecker', { update: update });
    };

    function Plugin () {
      global$3.add('grammerchecker', function (editor, pluginUrl) {
