tinymce.PluginManager.add('pageview', function(editor, url) {
    ...
    editor.ui.registry.addButton('pageview', {
        icon: "new-document",
        tooltip: 'New',
        onAction: function() {
            editor.execCommand('mceNewDocument');
    ...
    //editor.addShortcut('Meta+shift+N', '', 'mceNewDocument');
		...
		pages.forEach((page, pageIndex) => {
		    ...
		    if (pageHeight > EFFECTIVE_PAGE_HEIGHT + 100) { // Add buffer
		        ...
		        for (let i = 0; i < children.length; i++) {
		            ...
		            const relativeTop = childRect.bottom - pageRect.top;
		            
		            if (relativeTop > EFFECTIVE_PAGE_HEIGHT + 80) {
		                splitIndex = i;
		                break;
		            }
		        }
		        ...
		        if (splitIndex > 0) {
		            ...
		            setTimeout(() => {
		                ...
		                if (!nextPage.firstChild) {
		                    ...
		                    p.innerHTML = '&nbsp;';
		                    nextPage.appendChild(p);
		                }
		                
		                // Move cursor to new page
		                const firstElement = nextPage.firstChild;
		                editor.selection.select(firstElement);
		                editor.selection.collapse(true);
		                
		                // Create and set range for precise cursor positioning
		                const rng = editor.dom.createRng();
						rng.setStart(firstElement, firstElement.childNodes.length);
						rng.setEnd(firstElement, firstElement.childNodes.length);
	...
	// Register a command to paste text
	editor.addCommand('pasteText', function(text) {
		if (!text) return;
		
		// Insert the text at the current cursor position
		editor.insertContent(text);
		
		// Trigger the same overflow handling
		setTimeout(() => {
		    ensureCursorInPage();
	...
	editor.on('keydown', function(e) {
		...
		if (e.keyCode === 8 || e.keyCode === 46) { // Backspace or Delete key
		    ...
		    // If only one page exists, prevent deletion
		    if (pages.length === 1) {
		       ...
		       const isAllSelected = !selection.isCollapsed() && selection.getNode() === currentPage;
		        ...
		        if (isAtStart || isAllSelected) {
		            ...
		            // If all content was selected, clear it but keep the page structure
		            if (isAllSelected) {
		                currentPage.innerHTML = '<p><br></p>';
		                editor.selection.select(currentPage.firstChild);
		                editor.selection.collapse(true);
		            }
			...
			if (pages.length  > 1) {
				// generated by Claude "stop Tinymce to delete all empty lines in all empty pages."
				const currentPage = getCurrentPage();
				const pageIndex = Array.from(pages).indexOf(currentPage);
				const prevPage = pages[pageIndex - 1];
				const isPrevPageEmpty = prevPage.textContent.replace(/\u00A0/g, '').trim() === '';
				
				if (isPrevPageEmpty) {
					// Give TinyMCE something to stop at
					prevPage.lastChild.innerHTML = '&nbsp;';
				}
			}		    
	...
	// create new document when called
	const createNewDocument = function() {
		...
		const firstPage = editor.dom.create('div', {
		    'class': 'page',
		    'data-page': 1
		});
        ...
        // Move cursor to new page
        const firstElement = firstPage.firstChild;
        editor.selection.select(firstElement);
        editor.selection.collapse(true);
		...
		// Mark content as not dirty (optional - so user isn't prompted to save)
		editor.setDirty(false);
		
		// reset file handle
		window.fileHandle = null;
		window.tauriFilePath = null;
		window.titleUpdate._setTitle('Yadikar Text Editor');
	};

	// Register the newdocument command
	editor.addCommand('mceNewDocument', createNewDocument);
	...
	window.reapplyPageView = function(newContent) {
		...
		if (newContent && newContent.trim()) {
		    firstPage.innerHTML = newContent;
		} else {
		    // Create initial paragraph
		    const p = editor.dom.create('p');
		    p.innerHTML = '<br>';
        ...
        // make last element of last page selected
	    const pages = editor.getBody().querySelectorAll('.page');
	    const lastPage = pages[pages.length - 1];
        
        const lastElement = lastPage.lastChild;
        editor.selection.select(lastElement);
        editor.selection.collapse(true);
        
        // Create and set range for precise cursor positioning
        const rng = editor.dom.createRng();
		rng.setStart(lastElement, lastElement.childNodes.length);
		rng.setEnd(lastElement, lastElement.childNodes.length);
		editor.selection.setRng(rng);        
	};
});
