tinymce.PluginManager.add('file', function(editor, url) {
	...
	async function saveBlob(asNew, blob, filename, opts) {
	  // 1) Tauri desktop environment
		if (window.__TAURI__) {
			const { open: openDialog, save: saveDialog } = await import('@tauri-apps/plugin-dialog');
			...
			await writeFile(window.tauriFilePath, new Uint8Array(await blob.arrayBuffer()));
			return;
		}
