        ...
        editor.windowManager.open({
            ...
            onSubmit: function (e) {            
                var newfootnoteContent = e.getData().name,
					...
					titleContent = decodeURIComponent(fixFootnoteContent)
						.replace(/"/g, '&quot;')
						.replace(/</g, '&lt;')
						.replace(/>/g, '&gt;'),                    
                    htmlTemplate = '<span class="fnoteWrap" id="#wk_ft{FOOTNOTE_INDEX}" contenteditable="false"><button type="button" class="fnoteBtn" data-content="'+fixFootnoteContent+'" title="'+titleContent+'">{FOOTNOTE_INDEX}</button></span>',
                    totalFootNote = editor.getDoc().querySelectorAll('.fnoteBtn'),
                    totalCount = totalFootNote.length,
                    html;
                
 				if (isFootNotes && selectedNode) {
					// MODIFY existing footnote
					selectedNode.querySelector('.fnoteBtn').setAttribute('data-content', fixFootnoteContent);
					e.close();
					return;
				}                    
