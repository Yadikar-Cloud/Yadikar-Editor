...
(function () {
    ...
    var getLanguages = function (editor) {
      ...
      return editor.getParam('webspeechapi_language', defaultLanguages);
    };
    var getLanguage = function (editor) {
      var defaultLanguage = editor.getParam('language', 'en');
      return editor.getParam('spellchecker_language', defaultLanguage); // spellchecker_language param is not a BUG. that is the way to set webspeech api language to tinymce 5 language!
    };
		...
		var startSpeechRecognition = function (editor, startedState, currentLanguageState, SpeechRecognition) {
			...
			if(!startedState.get())
			{
				isListening = true; // MODIFY THIS LINE - set flag to true
				startRecognition(recognizer, currentLanguageState);			
			}
			else
			{
				isListening = false; // MODIFY THIS LINE - set flag to false
				stopRecognition(recognizer);				
			}
		}

		function startRecognition(recognizer, currentLanguageState) {
    ...
    var register = function (editor, startedState, currentLanguageState) {
      ...
      var startSpeechRecognition = function () {
      	startedState.set(!startedState.get());
        fireSpeechRecognitionStart(editor);
      };
      var buttonArgs = {
        tooltip: 'Speech Recognition',
        onAction: startSpeechRecognition,
        icon: 'microphone',
