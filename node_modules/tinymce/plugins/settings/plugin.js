tinymce.PluginManager.add('settings', function(editor, url) {
    ...
    const SETTINGS_VERSION = 3; // Increment this when schema changes
    ...
    function showSettingsDialog() {
        ...
        const dialogConfig = {
            ...
            onChange: async function(api) {
            	//console.log("settings is changed!");
            	let data = api.getData();
            	if(data.loadSystemFonts)
            	{
					extraData.systemFonts = await loadFontData();      	
            	}
            },
	...
	editor.ui.registry.addButton('addSystemFonts', {
	    ...
	    onAction: async function() {		    
	    	const currentSettings = getSettings();
	    	extraData.systemFonts = await loadFontData();
	    	applySettings({...currentSettings, ...extraData});
	    	tinymce.remove();
			initializeTinyMCE();		        
	    }
	});
