tinymce.PluginManager.add('universaldrive', function(editor, url) {
	...
	function showFileSelectionDialog(provider, mode) {
		...
		const dialogConfig = {
			...
			onSubmit: async function(api) {
				api.block('Processing...');
				
				// Also show progress in the editor
				editor.setProgressState(true);

				try {
					if(mode === 'save') {
						await performSave(provider, selectedFolderId);
					} else {
						await loadFileContent(selectedFile, provider);
					}
    ...
    async function performSave(provider, folderId) {
