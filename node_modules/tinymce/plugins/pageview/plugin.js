	...
	editor.on('keydown', function(e) {
		...
		if (e.keyCode === 8 || e.keyCode === 46) { // Backspace or Delete key
		    ...
		    if (pages.length === 1) {
				...
				// Create a range from the start of the page to current position
				const testRange = editor.dom.createRng();
				testRange.setStart(currentPage, 0);
				testRange.setEnd(selection.getRng().startContainer, selection.getRng().startOffset);

				// If the range is empty, we're at the start
				const isAtStart = testRange.toString().trim().length === 0;
